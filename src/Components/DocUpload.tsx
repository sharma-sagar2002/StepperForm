...
// const DocUpload: React.FC<DocUploadProps> = ({ readonly = false, register, errors }) => {
...
//                 <div className="mt-4">
//                     {(!readonly) && <h3 className="text-lg font-medium text-gray-700">Uploaded Documents:</h3>}
...
const DocUpload: React.FC<DocUploadProps> = ({ readonly = false, register, errors, setValue, getValues }) => {
    const [previewDocument, setPreviewDocument] = useState<File | null>(null);
    ...
    useEffect(() => {
        // Initialize uploaded documents from form state
    ...
    const handleRemoveDocument = (index: number) => {
        ...
        const updatedDocuments = uploadedDocuments.filter((_, i) => i !== index);
        setUploadedDocuments(updatedDocuments);
        setValue('documents', updatedDocuments); // Update the form state
    };
    ...
    const handleUpload = () => {
        ...
        if (previewDocument) {
            const newDocuments = [...uploadedDocuments, previewDocument];
            setUploadedDocuments(newDocuments);
            setValue('documents', newDocuments); // Update the form state
            setPreviewDocument(null); // Remove preview after upload
        }
    };
